...
// This is a fantastic function and greatly simplifies the work needed in x_form
// and bond_ptrsCompare()
...
{  
	struct molecule *molPtr = malloc(sizeof(struct molecule));
	if (molPtr == NULL) {
		exit(1);
	}
...
{	
	...
	molecule->atom_ptrs[molecule->atom_no] =
	&(molecule->atoms[molecule->atom_no]);
	molecule->atom_no++;
}
...
{	
	...
	molecule->bond_ptrs[molecule->bond_no] =
	&(molecule->bonds[molecule->bond_no]);
	molecule->bond_no++;
}
...
{	
	struct molecule *cpyMol = molmalloc(src->atom_max, src->bond_max);
	if (cpyMol == NULL) {
		exit(1);
	}
...
// bond_ptrsCompare needs to be tested again just to make sure the new changes
// haven't broken it
...
// we gotta modify mol_xform to work with the new bond structure using the new
// compute coordinates function above and fix the bugs in it
void mol_xform(struct molecule *molecule, double matrix[3][3]) 
{
	...
	// I think this is how the new modification for mol_xform is supposed to look
	// like, but I gotta test it more later
